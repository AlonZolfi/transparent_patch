...
from nn_modules import PatchApplier, DotApplier, Detections, WeightClipper, DistanceDots, NonPrintabilityScore
...
class TrainPatch:
    def __init__(self, mode):
        ...
        self.distance_dots = DistanceDots(
            self.config.dist_dot_weight,
            self.config.num_of_dots,
            self.img_size,
            self.config.radius)
    ...
    def set_to_device(self):
        ...
        self.distance_dots = self.distance_dots.to(device)
    ...
    def set_multiple_gpu(self):
        if torch.cuda.device_count() > 1:
            ...
            self.distance_dots = DP(self.distance_dots)
    ...
    def train(self):
        ...
        #                  amsgrad=True)
        optimizer = Adam([{'params': self.dot_applier.theta, 'lr': self.config.start_learning_rate},
                          {'params': self.dot_applier.colors, 'lr': 0.02}],
                         amsgrad=True)
        ...
        for epoch in range(self.config.epochs):
            ...
            for i_batch, (img_batch, lab_batch, img_names) in progress_bar:
                ...
                distant_dots = self.distance_dots(adv_patch)
                ...
                loss, loss_arr = self.loss_function(max_prob, cor_det, nps, distant_dots)  # calculate loss

                # save losses
                max_prob_loss += loss_arr[0].item()
                cor_det_loss += loss_arr[1].item()
                nps_loss += loss_arr[2].item()
                distant_dots_loss += loss_arr[3].item()
                ...
                if i_batch % 10 == 0 and self.writer is not None:
                    self.write_to_tensorboard(adv_patch, train_loss, max_prob_loss, cor_det_loss, nps_loss,
                                              distant_dots_loss, epoch_length, epoch, i_batch, optimizer)
                ...
                if i_batch + 1 == epoch_length:
                    self.last_batch_calc(adv_patch, alpha, epoch_length, progress_bar, prog_bar_desc,
                                         train_loss, max_prob_loss, cor_det_loss, nps_loss, distant_dots_loss,
                                         optimizer, epoch, i_batch)

                # clear gpu
                del img_batch, lab_batch, applied_batch, output_patch, max_prob, cor_det, distant_dots, loss
            ...
            scheduler.step(self.val_losses[-1] / epoch_length)
    ...
    def evaluate_loss(self, loader, adv_patch, alpha):
        ...
        for img_batch, lab_batch, img_names in loader:
            with torch.no_grad():
                ...
                # adv_patch, alpha = self.dot_applier(adv_patch, alpha)
                applied_batch = self.patch_applier(img_batch, adv_patch, alpha)
                output_patch = self.yolo(applied_batch)
                max_prob, cor_det = self.detections(lab_batch, output_patch, img_names)
                nps = self.non_printability_score(self.dot_applier.colors)
                distant_dots = self.distance_dots(adv_patch)
                batch_loss, _ = self.loss_function(max_prob, cor_det, nps, distant_dots)
                total_loss += batch_loss.item()

                del img_batch, lab_batch, applied_batch, output_patch, max_prob, cor_det, distant_dots
                torch.cuda.empty_cache()
    ...
    def plot_separate_loss(self):
        ...
        if self.config.dist_dot_weight != 0:
            plt.plot(epochs, self.distant_dots_losses, 'm', label='Distant dots loss')
    ...
    def loss_function(self, max_prob, cor_det, nps, distant_dots):
        max_prob_loss = torch.mean(max_prob)
        cor_det_loss = torch.mean(cor_det)
        return max_prob_loss + cor_det_loss + distant_dots + nps, [max_prob_loss, cor_det_loss, nps, distant_dots]
    ...
    def write_to_tensorboard(self, adv_patch, train_loss, max_prob_loss, cor_det_loss, nps_loss,
        ...
        self.writer.add_scalar('loss/dist_dots_loss', distant_dots_loss / (i_batch + 1), iteration)
    ...
    def save_config_details(self):
        ...
        row_to_csv = self.current_dir.split('/')[-1] + ',' + \
                     ...
                     str(self.config.start_learning_rate) + ',' + \
                     ...
                     str(self.config.dist_dot_weight) + ',' + \
                     str(self.config.num_of_dots) + ',' + \
                     str(self.config.radius) + ',' + \
...
def main():
    ...
    # patch_eval.create_yolo_true_labels(class_labels=[11])
    patch_eval.calculate()
